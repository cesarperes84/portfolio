...
function makeGiantRobot(scale = 1) {
  ...
  const abdomen = shadowify(new THREE.Mesh(new THREE.OctahedronGeometry(1.2 * scale, 1), mats.robotBody));
  abdomen.position.set(0, 4.8 * scale, -0.2 * scale);
  abdomen.scale.set(1.35, 1.0, 1.6);
  g.add(abdomen);

  const thorax = shadowify(new THREE.Mesh(new THREE.BoxGeometry(1.75 * scale, 1.1 * scale, 1.8 * scale), mats.robotWing));
  thorax.position.set(0, 4.95 * scale, 0.65 * scale);
  g.add(thorax);

  const core = shadowify(new THREE.Mesh(new THREE.OctahedronGeometry(0.45 * scale, 0), mats.robotCore));
  core.position.set(0, 5.0 * scale, 1.45 * scale);
  core.scale.set(1.2, 1.0, 0.8);
  g.add(core);

  for (const side of [-1, 1]) {
    for (let i = 0; i < 4; i++) {
      const zOffset = (i - 1.5) * 0.7 * scale;
      const rootX = side * (0.8 + i * 0.03) * scale;
      const rootY = 4.95 * scale - i * 0.06 * scale;
      const rootZ = 0.2 * scale + zOffset;

      const upperLeg = shadowify(new THREE.Mesh(new THREE.BoxGeometry(1.25 * scale, 0.18 * scale, 0.2 * scale), mats.robotAccent));
      upperLeg.position.set(rootX, rootY, rootZ);
      upperLeg.rotation.z = side * (0.55 + i * 0.06);
      upperLeg.rotation.y = side * (0.16 + i * 0.08);
      g.add(upperLeg);

      const lowerLeg = shadowify(new THREE.Mesh(new THREE.BoxGeometry(1.35 * scale, 0.15 * scale, 0.18 * scale), mats.robotBody));
      lowerLeg.position.set(side * (1.65 + i * 0.08) * scale, (3.9 - i * 0.14) * scale, rootZ + side * 0.1 * scale);
      lowerLeg.rotation.z = side * (-0.95 + i * 0.05);
      lowerLeg.rotation.y = side * (0.18 + i * 0.08);
      g.add(lowerLeg);

      const foot = shadowify(new THREE.Mesh(new THREE.ConeGeometry(0.14 * scale, 0.42 * scale, 4), mats.robotCore));
      foot.position.set(side * (2.2 + i * 0.1) * scale, (3.18 - i * 0.18) * scale, rootZ + side * 0.15 * scale);
      foot.rotation.z = side * Math.PI * 0.5;
      foot.rotation.x = Math.PI * 0.25;
      g.add(foot);
    }
  }
...
function buildFarBackground() {
  ...
  for (let i = 0; i < 9; i++) {
    ...
    for (let j = 0; j < puffCount; j++) {
      ...
      const puff = shadowify(
        new THREE.Mesh(new THREE.IcosahedronGeometry(radius, 1), cloudMat),
        false,
        false
      );
      puff.position.set(
        (j - (puffCount - 1) * 0.5) * (2.3 + Math.random() * 0.75),
        Math.random() * 0.25,
        THREE.MathUtils.randFloatSpread(0.75)
      );
      puff.scale.set(1.25 + Math.random() * 0.25, 0.92 + Math.random() * 0.08, 1.05 + Math.random() * 0.12);
      cloud.add(puff);
    }

    cloud.position.set(
      THREE.MathUtils.randFloatSpread(135),
      19 + Math.random() * 9,
      -28 + THREE.MathUtils.randFloatSpread(72)
    );
    cloud.scale.setScalar(0.9 + Math.random() * 0.25);
    skyRoot.add(cloud);
  }
}
buildFarBackground();
...
function cloneEnemyPalette() {
  const palettes = [
    { body: 0xe83d62, wing: 0x6934dc, canopy: 0xaedbff },
    { body: 0xf29f05, wing: 0x5036c2, canopy: 0xb7edff },
    { body: 0x00b5ad, wing: 0xff5a7d, canopy: 0xd1f4ff },
    { body: 0x7a58ff, wing: 0x39c16c, canopy: 0xdaf1ff },
    { body: 0xff6b35, wing: 0x1f7cff, canopy: 0xe8f7ff },
  ];
  const pick = palettes[Math.floor(Math.random() * palettes.length)];
  return {
    body: new THREE.MeshStandardMaterial({ color: pick.body, roughness: 0.5, flatShading: true }),
    wing: new THREE.MeshStandardMaterial({ color: pick.wing, roughness: 0.55, flatShading: true }),
    canopy: new THREE.MeshStandardMaterial({ color: pick.canopy, roughness: 0.25, metalness: 0.08, flatShading: true })
  };
}


function addEnemyDart(enemy, palette) {
  const body = shadowify(new THREE.Mesh(new THREE.OctahedronGeometry(0.58, 0), palette.body));
  body.scale.set(1.4, 0.58, 1);
  enemy.add(body);

  const leftWing = shadowify(new THREE.Mesh(createHorizontalWing(-1, 1.12, 0.98), palette.wing));
  leftWing.position.set(-0.16, -0.04, 0.03);
  enemy.add(leftWing);

  const rightWing = shadowify(new THREE.Mesh(createHorizontalWing(1, 1.12, 0.98), palette.wing));
  rightWing.position.set(0.16, -0.04, 0.03);
  enemy.add(rightWing);

  const tail = shadowify(new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.52, 0.16), palette.wing));
  tail.position.set(0, 0.28, 0.55);
  enemy.add(tail);
}

function addEnemyBlade(enemy, palette) {
  const body = shadowify(new THREE.Mesh(new THREE.BoxGeometry(1.15, 0.42, 0.96), palette.body));
  body.position.y = 0.02;
  enemy.add(body);

  const nose = shadowify(new THREE.Mesh(new THREE.ConeGeometry(0.34, 0.9, 4), palette.body));
  nose.rotation.x = Math.PI / 2;
  nose.position.set(0, 0.02, -0.82);
  enemy.add(nose);

  for (const side of [-1, 1]) {
    const wing = shadowify(new THREE.Mesh(createHorizontalWing(side, 1.45, 1.15), palette.wing));
    wing.position.set(side * 0.08, -0.02, 0.02);
    wing.rotation.z = side * 0.04;
    enemy.add(wing);

    const fin = shadowify(new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.65, 0.18), palette.wing));
    fin.position.set(side * 0.78, 0.22, 0.38);
    enemy.add(fin);
  }
}

function addEnemyFork(enemy, palette) {
  const core = shadowify(new THREE.Mesh(new THREE.OctahedronGeometry(0.5, 0), palette.body));
  core.scale.set(1.2, 0.7, 1.3);
  enemy.add(core);

  const cockpit = shadowify(new THREE.Mesh(new THREE.BoxGeometry(0.46, 0.18, 0.42), palette.canopy));
  cockpit.position.set(0, 0.18, -0.05);
  enemy.add(cockpit);

  for (const side of [-1, 1]) {
    const boom = shadowify(new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.14, 1.08), palette.wing));
    boom.position.set(side * 0.52, -0.02, 0.08);
    boom.rotation.y = side * 0.12;
    enemy.add(boom);

    const tip = shadowify(new THREE.Mesh(new THREE.TetrahedronGeometry(0.34, 0), palette.wing));
    tip.position.set(side * 1.05, -0.02, -0.22);
    tip.rotation.z = side * -0.55;
    enemy.add(tip);

    const rear = shadowify(new THREE.Mesh(new THREE.TetrahedronGeometry(0.28, 0), palette.wing));
...
function createEnemy(kind = null) {
  const enemy = new THREE.Group();
  const palette = cloneEnemyPalette();
  const shipTypes = ['dart', 'blade', 'fork'];
  const type = kind || shipTypes[Math.floor(Math.random() * shipTypes.length)];

  if (type === 'robot') {
    const robot = makeGiantRobot(0.28 + Math.random() * 0.06);
    enemy.add(robot);
    enemy.position.set(THREE.MathUtils.randFloat(-7.2, 7.2), 0.18, -82 - Math.random() * 12);
    enemy.userData = {
      speed: THREE.MathUtils.randFloat(7.2, 9.6) + wave * 0.45,
      baseX: enemy.position.x,
      baseY: enemy.position.y,
      phase: Math.random() * Math.PI * 2,
      swayX: THREE.MathUtils.randFloat(0.8, 1.8),
      swayY: 0,
      freq: THREE.MathUtils.randFloat(0.7, 1.1),
      homing: THREE.MathUtils.randFloat(1.0, 1.5),
      t: Math.random() * 2,
      shotTimer: THREE.MathUtils.randFloat(1.2, 2.2),
      burstLeft: 0,
      burstGap: 0,
      radius: 1.2,
      type,
  ...
  if (type === 'blade') addEnemyBlade(enemy, palette);
  else if (type === 'fork') addEnemyFork(enemy, palette);
  else addEnemyDart(enemy, palette);
...
function createSmokePuff(position) {
  const puff = shadowify(
    new THREE.Mesh(
      new THREE.IcosahedronGeometry(0.18 + Math.random() * 0.12, 1),
      new THREE.MeshStandardMaterial({ color: 0xbfc7d4, transparent: true, opacity: 0.7, roughness: 1, flatShading: true })
    ),
    false,
    false
  );
  puff.position.copy(position);
  puff.userData.life = 0.55 + Math.random() * 0.2;
  puff.userData.velocity = new THREE.Vector3(THREE.MathUtils.randFloatSpread(0.55), 0.75 + Math.random() * 0.45, THREE.MathUtils.randFloatSpread(0.45));
  puff.userData.spin = new THREE.Vector3(Math.random() * 1.5, Math.random() * 1.5, Math.random() * 1.5);
  puff.userData.fade = true;
  scene.add(puff);
  explosions.push(puff);
}

function damagePlayer(amountShield = 7, scorePenalty = 25, title = 'DAMAGE', text = 'A nave sofreu dano.') {
  if (gameOverSequence) return;
  shield -= amountShield;
  ...
  updateHud();
  if (shield <= 0) beginGameOverSequence();
}
...
function beginGameOverSequence() {
  if (gameOverSequence || gameOverTriggered) return;
  running = false;
  ...
  deathExploded = false;
  smokeTimer = 0;
  keys.left = keys.right = keys.up = keys.down = keys.shoot = keys.boost = false;
  setMessage('CRITICAL DAMAGE', 'A nave está caindo!');
}
...
function endGame() {
  ...
  setMessage('MISSION FAILED', 'A nave caiu durante a missão.');
  stopMusic();
}
...
function updateDeathSequence(dt) {
  ...
  player.rotation.z += dt * 1.8;
  player.rotation.x = THREE.MathUtils.lerp(player.rotation.x, -0.9, 0.06);
  player.position.y -= dt * (2.8 + deathTimer * 1.2);
  player.position.x += Math.sin(deathTimer * 7) * dt * 0.55;
  player.position.z += dt * 3.2;

  if (smokeTimer <= 0 && !deathExploded) {
    smokeTimer = 0.06;
    const smokePos = player.position.clone();
    smokePos.z += 0.8;
    smokePos.y += 0.1;
    createSmokePuff(smokePos);
  }

  if (!deathExploded && player.position.y <= 0.32) {
    deathExploded = true;
  ...
  if (deathExploded && deathTimer > 2.2) {
    endGame();
  }
}
...
function updateEnemies(dt) {
  ...
  if (spawnTimer <= 0) {
    const shouldSpawnRobot = robotsSpawnedThisWave < 4 && Math.random() < 0.22;
    createEnemy(shouldSpawnRobot ? 'robot' : null);
    spawnTimer = shouldSpawnRobot ? THREE.MathUtils.randFloat(2.2, 3.4) : Math.max(0.55, 1.25 - wave * 0.03);
  }
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    ...
    if (data.type === 'robot') {
      const desiredX = data.baseX + Math.sin(data.t * data.freq + data.phase) * data.swayX + player.position.x * 0.14;
      enemy.position.x = THREE.MathUtils.lerp(enemy.position.x, desiredX, dt * data.homing);
      enemy.position.y = 0.18 + Math.abs(Math.sin(data.t * 6 + data.walkPhase)) * 0.05;
      enemy.position.z += data.speed * dt * worldSpeed;
      enemy.rotation.y = THREE.MathUtils.lerp(enemy.rotation.y, (player.position.x - enemy.position.x) * 0.08, 0.08);
      enemy.rotation.z = THREE.MathUtils.lerp(enemy.rotation.z, -(enemy.position.x - prevX) * 3.2, 0.14);
    } else {
    ...
    if (enemy.position.z > -48 && enemy.position.z < 10) {
      if (data.burstLeft > 0) {
        ...
        if (data.burstGap <= 0) {
          ...
          data.burstLeft -= 1;
          data.burstGap = data.type === 'robot' ? 0.24 : 0.17;
        }
      } else if (data.shotTimer <= 0) {
        data.burstLeft = data.type === 'robot' ? THREE.MathUtils.randInt(1, 3) : THREE.MathUtils.randInt(2, 4);
        data.burstGap = 0;
        data.shotTimer = data.type === 'robot' ? THREE.MathUtils.randFloat(1.5, 2.8) : THREE.MathUtils.randFloat(1.2, 2.4);
      }
    }

    if (enemy.position.distanceTo(player.position) < (data.type === 'robot' ? 1.28 : 1.08)) {
      createExplosion(enemy.position.clone(), enemy);
      scene.remove(enemy);
      enemies.splice(i, 1);
      damagePlayer(data.type === 'robot' ? 14 : 10, data.type === 'robot' ? 45 : 35, 'DIRECT HIT', data.type === 'robot' ? 'Colisão com robô inimigo.' : 'Colisão direta com nave inimiga.');
      continue;
    }

    if (enemy.position.z > 16) {
      shield -= data.type === 'robot' ? 10 : 6;
      scene.remove(enemy);
      enemies.splice(i, 1);
      setMessage('SHIELD HIT', data.type === 'robot' ? 'Um robô inimigo atravessou a defesa.' : 'Um inimigo passou pela defesa.');
      updateHud();
      if (shield <= 0) beginGameOverSequence();
    }
  }
    ...
    for (let b = bullets.length - 1; b >= 0; b--) {
      ...
      if (enemy.position.distanceTo(bullet.position) < 0.82) {
        ...
        if (score > 0 && score % 700 === 0) {
          wave += 1;
          robotsSpawnedThisWave = 0;
          setMessage('WAVE UP', `Nível ${wave}. Mais velocidade e mais pressão.`);
        } else {
...
function updateExplosions(dt) {
  for (let i = explosions.length - 1; i >= 0; i--) {
    ...
    if (shard.userData.fade && shard.material) shard.material.opacity = Math.max(0, shard.userData.life);
...
function animate(now) {
  ...
  if ((running || gameOverSequence) && !paused) {
    if (running) {
      updatePlayer(dt);
      updateBullets(dt);
      updateEnemies(dt);
